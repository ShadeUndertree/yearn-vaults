...
def test_max_fees(gov, vault, token, TestStrategy, rewards, strategist):
    # performance fee should not be eq_higher than MAX
    ...
    # management fee should not be eq_higher than MAX
    ...
    strategy = strategist.deploy(TestStrategy, vault)
    with brownie.reverts():
        vault.addStrategy(strategy, 10 ** 18, 1000, FEE_MAX + 1, {"from": gov})

    # updateStrategyPerformanceFee should check for MAX FEE
    vault.addStrategy(strategy, 10 ** 18, 1000, 0, {"from": gov})
    with brownie.reverts():
        vault.updateStrategyPerformanceFee(strategy, FEE_MAX + 1, {"from": gov})
